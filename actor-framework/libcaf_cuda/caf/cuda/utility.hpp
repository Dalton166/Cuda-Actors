...
inline in<T> create_in_arg(const std::vector<T>& buffer) {
  return in<T>(std::make_shared<std::vector<T>>(buffer)); // copy constructor
}

template <typename T>
inline in_out<T> create_in_out_arg(const std::vector<T>& buffer) {
  return in_out<T>(std::make_shared<std::vector<T>>(buffer)); // copy constructor
}

template <typename T>
inline out<T> create_out_arg(const std::vector<T>& buffer) {
  return out<T>(std::make_shared<std::vector<T>>(buffer)); // copy constructor
}
